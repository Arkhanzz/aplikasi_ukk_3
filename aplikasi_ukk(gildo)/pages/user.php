...
if(isset($_GET['edit'])) {
    ...
    $e = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM tb_user WHERE id_user=$edit_id"));
}
?>
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div><h4 class="fw-bold mb-1 text-primary">Manajemen Pengguna</h4><p class="text-muted mb-0">Kelola data pengguna sistem</p></div>
...
<style>
    ...
    .btn-gradient-primary,.bg-gradient-primary{background:linear-gradient(135deg, #40425c 0%, #0e0846 100%);border:none;color:#fff}
    .btn-gradient-primary:hover{background:linear-gradient(135deg, #10548b 0%, #b8bcf1 100%);transform:translateY(-1px);box-shadow:0 4px 12px rgba(0,0,0,.15)}
    .table th{font-weight:600;letter-spacing:.5px}.table tbody tr:hover{background-color:rgba(102,126,234,.05)}
    .icon-circle{display:flex;align-items:center;justify-content:center;width:60px;height:60px}
    input:focus,select:focus{border-color:#667eea!important;box-shadow:0 0 0 .25rem rgba(102,126,234,.25)!important}
</style>