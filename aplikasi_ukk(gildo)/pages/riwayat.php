...
$riwayat = mysqli_fetch_all(mysqli_query($conn, "SELECT t.*, k.plat_nomor, k.pemilik, k.jenis_kendaraan, a.nama_area FROM tb_transaksi t JOIN tb_kendaraan k ON t.id_kendaraan=k.id_kendaraan LEFT JOIN tb_area_parkir a ON t.id_area=a.id_area WHERE t.status='keluar' ORDER BY t.waktu_keluar DESC"), MYSQLI_ASSOC);
        ...
        <div class="table-responsive"><table class="table table-hover align-middle mb-0" id="riwayatTable">
            ...
            <tbody>
                ...
                <tr data-tanggal="<?= date('Y-m-d',strtotime($r['waktu_keluar'])) ?>" data-jenis="<?= $jns ?>" data-plat="<?= strtolower($r['plat_nomor']) ?>" data-pemilik="<?= strtolower($r['pemilik']) ?>">
                    ...
                    <td><small class="text-muted"><i class="bi bi-geo-alt"></i></small> <?= $r['nama_area'] ?: '<em class="text-muted">Area dihapus</em>' ?></td>
