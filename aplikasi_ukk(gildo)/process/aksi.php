...
elseif($aksi=='hapus_area'){
    ...
    // Check FK: transaksi menggunakan area ini
    $chk = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as cnt FROM tb_transaksi WHERE id_area=$id"));
    if($chk['cnt'] > 0) 
        exit("<script>alert('Tidak bisa hapus area - masih ".intval($chk['cnt'])." transaksi menggunakannya');history.back();</script>");
