...
elseif($aksi=='tambah_tarif'){
    ...
    $allowedJenis = ['mobil','motor','lainnya'];
    $jenis = trim($_POST['jenis']);
    if(!in_array($jenis, $allowedJenis))
        exit("<script>alert('Jenis kendaraan tidak valid');history.back();</script>");
...
elseif($aksi=='hapus_tarif'){
    ...
    // Check FK: transaksi aktif yang menggunakan tarif ini
    ...
    // Set id_tarif to NULL for all transactions (completed ones) to avoid foreign key constraint
    mysqli_query($conn,"UPDATE tb_transaksi SET id_tarif=NULL WHERE id_tarif=$id");
...
elseif($aksi=='konfirmasi_keluar'){
    ...
    if($transaksi['id_tarif']) {
        ...
        // Tarif sudah dihapus, gunakan biaya default atau 0
        $total = 0;
    }
