<?php
$e=null;
if(isset($_GET['edit'])) $e=mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM tb_tarif WHERE id_tarif='$_GET[edit]'"));
$count = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM tb_tarif"));
?>
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div><h4 class="fw-bold mb-1 text-dark">Manajemen Tarif Parkir</h4><p class="text-muted mb-0 small">Kelola tarif parkir berdasarkan jenis kendaraan</p></div>
    ...
    <div class="card border-0 shadow-lg mb-5" style="border-radius:20px; overflow:hidden;">
        ...
        <div class="card-body p-4"><form method="POST" action="process/aksi.php" class="row g-4">
            ...
            <div class="col-md-5"><label class="form-label fw-semibold text-muted small">Jenis Kendaraan</label><div class="input-group input-group-lg"><span class="input-group-text bg-light border-end-0"><i class="bi bi-car-front text-success"></i></span><input class="form-control border-start-0 ps-2" name="jenis" placeholder="Mobil, Motor, dll." value="<?= $e['jenis_kendaraan']??'' ?>" required style="height:50px;"></div></div>
            <div class="col-md-5"><label class="form-label fw-semibold text-muted small">Tarif per Jam</label><div class="input-group input-group-lg"><span class="input-group-text bg-light border-end-0"><i class="bi bi-clock text-success"></i></span><input class="form-control border-start-0 ps-2" name="tarif" placeholder="Masukkan angka" value="<?= $e['tarif_per_jam']??'' ?>" required style="height:50px;"><span class="input-group-text bg-light">/ jam</span></div></div>
...
<style>
    ...
    .btn-gradient-success,.bg-gradient-success{background:linear-gradient(135deg, #fff00f 0%, #dab45c 100%);border:none;color:#fff}
    .btn-gradient-success:hover{background:linear-gradient(135deg, #fff00f 0%, #dab45c 100%);transform:translateY(-1px);box-shadow:0 4px 12px rgba(0,0,0,.15)}
    .table th{font-weight:600;letter-spacing:.5px}.table tbody tr:hover{background-color:rgba(133, 135, 25, 0.3)}
    .icon-circle{display:flex;align-items:center;justify-content:center;width:60px;height:60px}
    input:focus{border-color:#00b09b!important;box-shadow:0 0 0 .25rem rgba(164, 176, 0, 0.25)!important}
    .alert{border-radius:12px;background:rgba(210, 240, 13, 0.1);border:none}
</style>