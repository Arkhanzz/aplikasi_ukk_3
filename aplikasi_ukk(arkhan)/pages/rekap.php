...
<style>
    ...
    .btn-gradient-primary, .bg-gradient-primary { background: linear-gradient(135deg, #cccc37 0%, #dab45c 100%); color: white; border: none; }
    .bg-gradient-success { background: linear-gradient(135deg, #516426 0%, #dab45c 100%); color: white; }
    .bg-gradient-warning { background: linear-gradient(135deg, #8a7f0b 0%, #dab45c 100%); color: white; }
    .bg-gradient-info { background: linear-gradient(135deg, #618504 0%, #dab45c 100%); color: white; }
    .table-hover tbody tr:hover { background-color: rgba(208, 234, 102, 0.05); }
</style> 
...
$biayaRealtime = $jamTagih * $r['tarif_per_jam'];
...
function exportExcel() {
    ...
    <?php if(!empty($data_rekap)): foreach($data_rekap as $r):
        ...
        if(!empty($r['waktu_keluar'])){
            ...
            $waktu_keluar = '-';
            $selisih = time() - $masuk;
            $biaya = ceil($selisih / 3600) * $r['tarif_per_jam'];
            if($biaya < $r['tarif_per_jam']) $biaya = $r['tarif_per_jam'];
        }
